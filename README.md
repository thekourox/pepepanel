...
## 🐧 نصب سریع و خودکار روی سرور لینوکس (پیشنهاد می‌شود)

اگر می‌خواهید با یک کلیک تمام مراحل دانلود، نصب و تنظیمات (به همراه دریافت نام کاربری و رمز عبور) انجام شود، این دستور یک‌خطی را در سرور لینوکس خود وارد کنید:

```bash
bash <(curl -Ls https://YOUR_MIRROR_DOMAIN/install.sh)
```
*(پس از اجرای دستور، از شما License Key خواسته می‌شود. اگر می‌خواهید کاملاً اتوماتیک نصب شود رمز را به عنوان پارامتر ارسال کنید: `bash <(...) "SuperSecretKey2026!"`)*

---

## 🛠 نصب دستی در سرور لینوکس (با فایل Manager)

اگر قصد دارید فایل‌ها را به صورت دستی روی سرور کپی کنید، یک اسکریپت منیجر کامل به زبان Bash برای شما آماده شده است.
